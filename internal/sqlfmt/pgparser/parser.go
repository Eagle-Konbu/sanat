...
// Parser parses a token stream produced by a Lexer into pgast nodes.
//
...
// entry point via recoverParseError, the same shape internal/sqlfmt/parser
// uses for its MySQL grammar.
type Parser struct {
	lex      *Lexer
	tok      Token
	peekTok  Token
...
// statement from a function whose result type isn't void: fail's panic means
// the zero value it returns is never actually reached, but Go still requires
// a value of the right type to satisfy the return statement.
...
func ParseExpr(input string) (expr pgast.Expr, err error) {
